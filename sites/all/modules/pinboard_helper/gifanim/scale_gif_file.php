<?php
function scaleGifFile($fileSrc, $w, $h, $width, $height, &$saveTo, $resizemethod = 1){
	$delays = array(5);

      $type = 1;
      $fimg = file_get_contents(drupal_realpath($fileSrc));
			if($type == 1 && is_ani($fimg)){
				$gif = new GIFDecoder($fimg);
			    ...
			    if(! ($oldimg = $fileSrc)) return false;
...
function scaleGifFileNOP($fileSrc, $w, $h, $width, $height, &$saveTo, $resizemethod = 1){
      $type = 1;
      $fimg = file_get_contents(drupal_realpath($fileSrc));
      $saveTo = $fimg;
      return true;
...
function is_ani($fileSrc) {
  if (empty($fileSrc)) return FALSE;
  if (!is_string($fileSrc)) return FALSE;
	return (bool)preg_match('/\x00\x21\xF9\x04.{4}\x00(\x2C|\x21)/s', $fileSrc, $m);
}